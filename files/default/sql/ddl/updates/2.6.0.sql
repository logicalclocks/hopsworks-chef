...
ALTER TABLE `hopsworks`.`training_dataset` ADD CONSTRAINT `td_model_feature_fk` FOREIGN KEY
    (`model_feature_id`) REFERENCES `model_feature` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION;
ALTER TABLE `hopsworks`.`training_dataset_join` ADD COLUMN `model_feature_id` INT(11) DEFAULT NULL;
ALTER TABLE `hopsworks`.`training_dataset_join` ADD CONSTRAINT `tdj_model_feature_fk` FOREIGN KEY (`model_feature_id`) REFERENCES
    `model_feature` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION;
ALTER TABLE `hopsworks`.`training_dataset_feature` ADD COLUMN `model_feature_id` INT(11) DEFAULT NULL;
ALTER TABLE `hopsworks`.`training_dataset_feature` ADD CONSTRAINT `tdf_model_feature_fk` FOREIGN KEY (`model_feature_id`)
    REFERENCES `model_feature` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION;
ALTER TABLE `hopsworks`.`feature_store_activity` ADD COLUMN `model_feature_id` INT(11) DEFAULT NULL;
ALTER TABLE `hopsworks`.`feature_store_activity` ADD CONSTRAINT `fsa_model_feature_fk` FOREIGN KEY (`model_feature_id`) REFERENCES
    `model_feature` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION;
